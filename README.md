...
```java
     ...
     * @return
     */
    public String poiExport2(HttpServletResponse response, String[] titles) {
        long startTime = System.currentTimeMillis();
        // 获取导出数据
        List<CbecOrder> orderList = getData();
        ...
        for (int j = 0; j < splitList.size(); j++) {
            ...
            // 添加一个 sheet
            Sheet sheet = wb.createSheet("订单数据" + (j+1));
            // 添加表头
        ...
        try (ByteArrayOutputStream os = new ByteArrayOutputStream();){
            ...
            // 设置 response 参数
            response.setContentType("application/vnd.ms-excel;charset=utf-8");
            String fileName = new String("poi2订单导出.xlsx".getBytes("UTF-8"), "ISO-8859-1");
            response.setHeader("Content-Disposition", "attachment;filename=" + fileName);
...
| 性能指标\导出方案 | POI-HSSFWorkbook | POI-SXSSFWorkbook | EasyExcel |
| ----------------- | ---------------- | ----------------- | --------- |
| **耗时**          | 49秒             | 46秒              | 57秒      |
| **最大使用CPU**   | 60%              | 47%               | 52%       |
| **平均使用内存**  | 1.5GB            | 1.9GB             | 1.4GB     |
| **导出文件大小**  | 85MB             | 16.8MB            | 17.1MB    |
